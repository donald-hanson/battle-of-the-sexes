DefaultHud = {
	...
	Draw : function()
	{
		Hud.DrawBigString(320, 240, "Hello World!", 0.75, Constants.Hud.Alignment.Center);
...
*/
	...
	{
	    DefaultHud._DrawAmmo(ps);
	
        DefaultHud._DrawHealth(ps);
        DefaultHud._DrawArmor(ps);
	},
	
	_DrawAmmo : function(ps)
	{
	    var l = [];
	    ...
	    for (var index = 0; index < l.length; index++)
	    {
		    ...
		    Hud._SetColor([1.0, 0.69, 0.0, 1.0]);
		    Hud._DrawField(0, y, 3, v);
		    Hud._SetColor();
		    ...
		    Hud._DrawPic(x, y, w, h, icon);

		    y -= Constants.Hud.Char.Height;
	    }
	},
	
	_DrawHealth : function(ps)
	{
		...
		var health = ps.stats[Constants.Stats.Health];
		{
		    if (health > 100)
			    ...
			    colorIndex = 1;
		    Hud._SetColor(colors[colorIndex]);
		    Hud._DrawField(0, y, 3, health);
		    Hud._SetColor();	
        }
		...
		//Draw Health Model
		{
			...
			var w = Constants.Hud.Icon.Size;
			var h = Constants.Hud.Icon.Size;
			Hud._Draw3DModel(x, y, w, h, Game.Static.Media.healthSphereModel, 0, origin, angles);
			Hud._Draw3DModel(x, y, w, h, Game.Static.Media.healthCrossModel, 0, origin, angles);
		}			
	},
	
	_DrawArmor : function(ps)
	{
        ...
        //Draw Armor Model
		{
			...
			var w = Constants.Hud.Icon.Size;
			var h = Constants.Hud.Icon.Size;
			Hud._Draw3DModel(x, y, w, h, Game.Static.Media.armorModel, 0, origin, angles);
		}			
	},
	...
	DrawBottomRight : function(ps)
	{
	    ...
	    Hud._DrawBigStringColor(640, bluPosition, bluMsg, bluColor, Constants.Hud.Alignment.Right);	    
	    Hud._DrawBigStringColor(640, redPosition, redMsg, redColor, Constants.Hud.Alignment.Right);
	},
	...
	DrawTopRight: function(ps)
	{
	    ...
	    for (var n in Constants.Class)
	        if (cls == Constants.Class[n])
	            Hud._DrawBigString(640, 0, n, 1.0, Constants.Hud.Alignment.Right);

        var lvl = ps.persistant[Constants.Persistant.Level];
        Hud._DrawBigString(640, Constants.Hud.BigChar.Height, "Level " + lvl.toString(), 1.0, Constants.Hud.Alignment.Right);
	},
	...
	DrawCrosshair : function(ps)
	{
	    ...
	    var a = Hud._AdjustFrom640(x,y,w,h);
        ...
        if (Sys.Cvar.GetInt("cg_crosshairHealth") > 0)
            Hud._SetColor(Hud._GetColorForHealth());
        else
            Hud._SetColor();
